...
/*
	Return a string-bases representation based on the attribute type.
*/
...
{
	level = 0;
	//Initilaize the SDK manage. This object handles memory management.
	...
	//Create a new scene so thta it can be populated by the imported file.
...
void processNode(FbxNode *node, MeshData *meshData)
{
	...
	cout << "Node: " << nodeName <<" "<< "Position: " << translation[0] << " " << translation[1] << " " << translation[2] << " " << "Rotation: " << rotation[0] << " " << rotation[1] << " " << rotation[2] << " " << "Scale: " << scaling[0] << " " << scaling[1] << " " << scaling[2] << endl;
	...
	for (int iPolygon = 0; iPolygon < mesh->GetPolygonCount(); iPolygon++)
	{
		for (unsigned IPolygonVertex = 0; IPolygonVertex < 3; IPolygonVertex++)
		{
			...
			if (fbxLayerUV)
			{
				...
				{
				case FbxLayerElement::eByControlPoint:iUVIndex = fbxCornerIndex;
					break;
